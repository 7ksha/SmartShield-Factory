...
class ISQLite(ABC):
    ...
    Any sqlite db that smartshield connects to should use thisinterface for
    ...
    def __init__(self, name: str, main_pid: int, db_file: str):
        ...
        :param main_pid: the pid of smartshield.py, used to create a lock file to
         make sure only 1 lockfile is created per smartshield run per sqlite db
        """
        ...
        # non root smartshield modules to use this lock. (because some modules
        # drop privs when smartshield runs with -p and others dont)
    ...
    def _init_flock(self, name: str, main_pid: int, current_user_uid: int):
        ...
        # its own lock file that is different from smartshield' main sqlite db lockfile
        username = os.getenv("USER") or "unknown"
        # we're using the username and pid to create a unique lock file per
        # smartshield run, so that multiple instances of smartshield can run at the
        # same time
        self.lockfile_path = os.path.join(
            utils.smartshield_locks_dir, f"{username}_{main_pid}_{name}.lock"
        )
        try:
    ...
    def execute(self, query: str, params=None) -> None:
        ...
        Since smartshield uses python3.10, we can't use autocommit here. we have
        to do it manually
