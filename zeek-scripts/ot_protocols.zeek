...
event udp_request(u: connection)
{
    if (u$id$resp_p == 34964/udp) {
        ...
        ++profinet_dcp_count[src];

        if (profinet_dcp_count[src] >= profinet_threshold) {
            NOTICE([$note=OTProtocolAnomaly,
                    $conn=u,
                    $msg=fmt("PROFINET DCP discovery flood from %s: "
                             "%d req/min",
                             src, profinet_dcp_count[src]),
                    $identifier=fmt("profinet-flood-%s", src)]);
        }
    }
}
    ...
    if (u$id$resp_p == 319/udp || u$id$orig_p == 319/udp) {
        ...
        NOTICE([$note=OTProtocolAnomaly,
                ...
                $msg=fmt("PTP (IEEE 1588) traffic from %s — verify it is "
                         "a known grandmaster clock", src),
                $identifier=fmt("ptp-traffic-%s", src)]);
    }
}
